...
// SafeMove attempts to move with proper error handling and room detection
...
func (gs *GameState) getLocationDescription() string {
	...
	// Items in location - use pre-formatted item names directly
	items := gs.GetItemsAtCurrentLocation()
	if len(items) > 0 {
		formattedItemNames := make([]string, len(items))
		for i, itemID := range items {
			if item, exists := gs.Items[itemID]; exists {
				// Get the already-formatted display name
				formattedItemNames[i] = gs.GetDisplayName(item.Name)
			}
		}
...
func (gs *GameState) GetPlayerStatus() string {
	...
	result.WriteString(gs.Formatter.FormatHeader(models.Text{
		Chinese: "角色状态",
		Pinyin:  "Jue Se Zhuang Tai",
		English: "Character Status",
	}))

	result.WriteString(fmt.Sprintf("%s: %s\n",
		gs.Formatter.formatInline("名称", "Ming Cheng", "Name"),
		gs.Player.Name))

	result.WriteString(fmt.Sprintf("%s: %d\n",
		gs.Formatter.formatInline("等级", "Deng Ji", "Level"),
		gs.Player.Level))
	...
	result.WriteString(fmt.Sprintf("%s: %d/%d %s %s\n",
		gs.Formatter.formatInline("生命", "Sheng Ming", "Health"),
	...
	result.WriteString(fmt.Sprintf("%s: %d / %d\n",
		gs.Formatter.formatInline("经验", "Jing Yan", "Experience"),
		gs.Player.ExperiencePoints, expNeeded))

	currentInv, maxInv := gs.Player.GetInventoryCapacity()
	result.WriteString(fmt.Sprintf("%s: %d / %d\n",
		gs.Formatter.formatInline("背包容量", "Bei Bao Rong Liang", "Inventory"),
		currentInv, maxInv))
	...
	if gs.Player.HasEquippedWeapon() {
		if weapon, exists := gs.Items[gs.Player.GetEquippedWeaponID()]; exists {
			result.WriteString(fmt.Sprintf("  %s: %s (+%d)\n",
				gs.Formatter.formatInline("武器", "Wu Qi", "Weapon"),
	...
	if gs.Player.HasBackpackEquipped() {
		if backpack, exists := gs.Items[gs.Player.GetEquippedBackpackID()]; exists {
			result.WriteString(fmt.Sprintf("  %s: %s (+%d)\n",
				gs.Formatter.formatInline("背包", "Bei Bao", "Backpack"),
				gs.GetDisplayName(backpack.Name),
...
func (gs *GameState) GetInventoryDisplay() string {
	...
	for _, itemID := range gs.Player.Inventory {
		if item, exists := gs.Items[itemID]; exists {
			...
			if gs.Player.GetEquippedWeaponID() == itemID {
				equipped = gs.Formatter.formatInline(" [已装备]", " [yi zhuang bei]", " [equipped]")
			}
	...
	result.WriteString(gs.Formatter.formatInline(
		...
		fmt.Sprintf("Rong Liang: %d / %d", currentInv, maxInv),
...
func (gs *GameState) SafeUnequipBackpack() (string, error) {
	...
	if !exists {
		...
		return "取下了背包（数据异常）", nil
	}

	sizeBonus := backpack.GetSizeBonus()

	success, decrease, message := gs.Player.UnequipBackpackWithBonus(sizeBonus)
	if !success {
		return "", errors.New(errors.ErrTypeInventory, message, message, message)
	}
	...
	return gs.Formatter.formatInline(
		fmt.Sprintf("取下了 %s，背包容量 -%d (当前: %d)", backpack.Name.Chinese, decrease, gs.Player.InventorySize),
